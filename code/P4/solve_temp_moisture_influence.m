...
load("wisconsin.mat");
load("sacramento.mat");                     % semi-arid
load("seattle.mat");                        % temperate
load("boston.mat");                         % arboreal
load("hawaii.mat");                         % tropical forest
load("lasvegas.mat");                       % arid
...
for t = time_line(2:end)
    ...
    for specy = participated_species
        ...
        % T(mod(index-1, 4*365)+1,1) = T(mod(index-1, 4*365)+1,1) + 0.05;
        Fh = ah * exp(-(((H - bh) / ch)^2));
        k3 = k1 / Ft0 / Fh0;
